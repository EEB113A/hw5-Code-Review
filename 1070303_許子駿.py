from linked_list import Node, DoubleLinkedList

def combine(l1, l2):
    # <將你的程式碼寫在這邊>
    #設定4個點分別為l1的頭跟尾以及l2的頭跟尾
    l1_tail = l1.head.pre
    l1_head = l1.head
    l2_tail = l2.head.pre
    l2_head = l2.head
    #使l1的頭連接到l2的尾、l2的尾連接到l1的頭
    l1_head.pre = l2_tail#本來的0指向3，變為0指向13
    l2_tail.next = l1_head#本來的13指向10，變為13指向0
    #使l1的尾連接到l2的頭、l2的頭連接到l1的尾
    l1_tail.next = l2_head#本來的3指向0，變為3指向10
    l2_head.pre = l1_tail#本來的10指向13，變為10指向3

if __name__=="__main__":
    # 建立 LL1 環狀雙向鏈結串列
    LL1 = DoubleLinkedList()
    LL1.insertHead(0)
    LL1.insert(0, 1)
    LL1.insert(1, 2)
    LL1.insert(2, 3) # 此時 LL1 為 [0 1 2 3] 的環狀雙向鏈結串列

    # 建立 LL2 環狀雙向鏈結串列
    LL2 = DoubleLinkedList()
    LL2.insertHead(10)
    LL2.insert(10, 11)
    LL2.insert(11, 12)
    LL2.insert(12, 13) # 此時 LL2 為 [10 11 12 13] 的環狀雙向鏈結串列

    # 使用 combine 函式合成2個環狀雙向鏈結串列，並改變 LL1 物件本身
    combine(LL1, LL2)
    LL1.printForward()  # 順向印出(next)
    LL1.printBackward() # 反向印出(pre)
    # Forward : 0 1 2 3 10 11 12 13
    # Backward : 13 12 11 10 3 2 1 0
    cur = LL1.head
    while cur.pre!=LL1.head:
        print(cur.data)
        cur = cur.pre
    print(cur.data)


#留言板